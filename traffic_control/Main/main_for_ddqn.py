...
class DeepRQNetWork(object):
    ...
    def _build_net(self):
        ...
        embedding = keras.layers.Embedding(input_dim=2, output_dim=8, input_length=1)(phase)
        ...
        dense_p = concatenate([dense_p, embedding])
        ...
        dense_a = concatenate([dense_a, embedding])
    ...
    def forward_e(self, s):
        ...
        sn = np.reshape(s[0], newshape=[-1,  self.size])
    ...
    def optimizer(self):
        ...
        train = K.function([self.model.input[0], self.model.input[1], a, y], [loss, error], updates=updates)
        return train
    ...
    def learnp(self):
        ...
        # phase_ = [[bitch[1] for bitch in batch[0]] for batch in bran_batch]
        phase_ = [batch[3][1] for batch in bran_batch]
        phase_ = np.reshape(phase_, newshape=[-1, 1])
        # state_ = np.reshape(state_, newshape=[-1, self.road_size, self.road_size, 1])
        q_next_t = self.target_model.predict([state_, phase_])[:, 0:2]
        if self.DDQN:
            q_next_e = self.model.predict([state_, phase_])[:, 0:2]
        ...
        self.cost, abs_errors = self.optimizer([state, phase, action, q_target])
    ...
    def learna(self):
        ...
        phase_ = [batch[3][1] for batch in bran_batch]
        phase_ = np.reshape(phase_, newshape=[-1, 1])
        q_next_t = self.target_model.predict([state_, phase_])[:,-2:]
        if self.DDQN:
            q_next_e = self.model.predict([state_, phase_])[:,-2:]
            max_act_next = np.argmax(q_next_e, axis=1)
            selected_q_next = q_next_t[range(self.batch_size), max_act_next]
        else:
            selected_q_next = np.max(q_next_t, axis=1)
